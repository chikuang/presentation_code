...
df_equ <- nba_2019_df %>% group_by(grid) %>% 
  rename(phat_1 = phat_espn, phat_2 = IP) %>% 
...
# Plotting ---------------------------------------------------------------
my_label <- TeX(sprintf("P-value of test of $H_0:$ $||\\Delta_N||^2=0$ is $%.0f$", p_hat))
...
df_equ %>% 
  ...
  geom_segment(aes(x = 1.02, xend = 1.02, y = 0, yend = -0.20), colour = "purple", 
               arrow = arrow(length = unit(0.45, "cm")), size = 2) +
  geom_segment(aes(x = 1.02, xend = 1.02, y = 0, yend = 0.05), colour  = "orange", 
               arrow = arrow(length = unit(0.45, "cm")), size = 2) + 
